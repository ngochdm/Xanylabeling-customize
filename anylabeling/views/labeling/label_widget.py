...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        self.auto_navigate_direction = 1
        ...
        # MARK: ngochdm 
        keep_zoom_position = action(
            self.tr("Keep Zoom Position"),
            lambda x: self._config.update({"keep_zoom_position": x}),
            tip=self.tr("Keep previous zoom position"),
            checkable=True,
            checked=self._config["keep_zoom_position"],
            enabled=True,
        )
    ...
    # MARK: ngochdm 
    def get_zoom_view_rect(self):
        if self.filename is None or self.canvas.pixmap is None:
            return None

        hbar = self.scroll_bars[Qt.Horizontal]
        vbar = self.scroll_bars[Qt.Vertical]

        top_left = QtCore.QPoint(hbar.value(), vbar.value())
        bottom_right = QtCore.QPoint(
            hbar.value() + hbar.pageStep(),
            vbar.value() + vbar.pageStep(),
        )

        p1 = self.canvas.transform_pos(top_left)
        p2 = self.canvas.transform_pos(bottom_right)
        ...
        return (
            ...
            p2.x(),
            p2.y(),
            self.zoom_widget.value(),
        )

    def set_zoom_view_rect(self, view_rect):
        if not view_rect:
            return

        x1, y1, _x2, _y2, zoom_value = view_rect
        ...
        h_value = (x1 + offset.x()) * scale
        v_value = (y1 + offset.y()) * scale

        self.set_scroll(Qt.Horizontal, h_value)
        self.set_scroll(Qt.Vertical, v_value)
    ...
    def load_file(self, filename=None):  # noqa: C901
        ...
        # MARK: ngochdm 
        prev_zoom_view_rect = None
        if self._config["keep_zoom_position"]:
            prev_zoom_view_rect = self.get_zoom_view_rect()
        ...
        # set scroll values
        if self._config["keep_zoom_position"]:
            self.set_zoom_view_rect(prev_zoom_view_rect)
        else:
    ...
    def _auto_navigate_once(self):
        if (
            not self.auto_navigate_running
            or self.auto_navigate_remaining_steps <= 0
        ):
            self.stop_auto_navigate()
            return
        ...
        if self.auto_navigate_direction > 0:
            self.open_next_image()
        else:
            self.open_prev_image()
